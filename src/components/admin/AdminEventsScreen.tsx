...
export const AdminEventsScreen: React.FC = () => {
  const { adminEvents, toggleEventPublication, deleteEvent, navigateTo } = useApp();
  ...
  const filtered = adminEvents.filter(item => {
  ...
  const publishedCount = adminEvents.filter(e => e.published !== false).length;
  const unpublishedCount = adminEvents.length - publishedCount;

  return (
    <div className="min-h-screen bg-[#F4F6F4] text-[#1E2522] flex flex-col font-sans pb-12">
      <AdminHeader
        title="Eventos"
        subtitle={`${adminEvents.length} eventos cadastrados`}
      ...
      <main className="p-4 space-y-4 max-w-md mx-auto w-full">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            type="button"
            onClick={() => navigateTo('admin_new_event')}
        ...
        <div className="space-y-3">
          {filtered.length === 0 ? (
            <div className="p-8 text-center bg-white rounded-2xl border border-[#E2E8F0]">
              <Calendar size={36} className="mx-auto text-[#A0AEC0] mb-2" />
              <h3 className="text-sm font-bold text-[#1A202C]">Nenhum evento encontrado</h3>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 text-[11px] text-[#718096] mt-1">
                        <span className="flex items-center gap-1 font-bold text-[#1B3F18]">
                          <Calendar size={11} />
                          {item.date}
                        </span>
                      ...
                      <div className="flex items-center gap-1 text-[11px] text-[#718096] mt-1 truncate">
                        <MapPin size={11} className="shrink-0 text-[#1B3F18]" />
                  ...
                  <div className="flex items-center justify-between gap-2 pt-3 mt-3 border-t border-[#F0F0F0]">
                    <button
                      type="button"
                      onClick={() => toggleEventPublication(item.id)}
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        if (confirm(`Deseja realmente remover o evento "${item.title}"?`)) {
                          deleteEvent(item.id);
                        }
                      }}
      ...
      {showNoticeModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-[#E2E8F0] animate-scaleUp text-center">
            <div className="w-14 h-14 rounded-2xl bg-purple-50 text-purple-800 flex items-center justify-center mx-auto mb-3">
              <Info size={28} />
            </div>
