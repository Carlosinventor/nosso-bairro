...
export const AdminCampaignsScreen: React.FC = () => {
  const { adminCampaigns, toggleCampaignPublication, deleteCampaign, navigateTo } = useApp();
  ...
  const filtered = adminCampaigns.filter(item => {
  ...
  const publishedCount = adminCampaigns.filter(c => c.published !== false).length;
  const unpublishedCount = adminCampaigns.length - publishedCount;

  return (
    <div className="min-h-screen bg-[#F4F6F4] text-[#1E2522] flex flex-col font-sans pb-12">
      <AdminHeader
        title="Campanhas"
        subtitle={`${adminCampaigns.length} campanhas cadastradas`}
        badge={`${publishedCount} ativas`}
      />
      ...
      <main className="p-4 space-y-4 max-w-md mx-auto w-full">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            type="button"
            onClick={() => navigateTo('admin_new_campaign')}
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Buscar por título, organização ou meta..."
            value={searchTerm}
        ...
        <div className="space-y-3">
          {filtered.length === 0 ? (
            <div className="p-8 text-center bg-white rounded-2xl border border-[#E2E8F0]">
              <HeartHandshake size={36} className="mx-auto text-[#A0AEC0] mb-2" />
              <h3 className="text-sm font-bold text-[#1A202C]">Nenhuma campanha encontrada</h3>
              <p className="text-xs text-[#718096] mt-1">Tente pesquisar com outros termos.</p>
            </div>
          ) : (
            filtered.map(item => {
              ...
              return (
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1">
                        ...
                        >
                          ...
                          <span>{isPublished ? 'Publicada' : 'Despublicada'}</span>
                        </span>
                      </div>

                      <h3 className="text-[14px] font-bold text-[#1A202C] mt-1 line-clamp-1 leading-snug">
                        {item.title}
                      </h3>

                      {/* Progress bar */}
                      <div className="mt-2">
                        <div className="flex items-center justify-between text-[10.5px] font-bold text-[#4A5568] mb-1">
                          <span className="flex items-center gap-1">
                            <Target size={11} className="text-[#1B3F18]" />
                            {item.goal}
                          </span>
                          <span className="text-[#1B3F18]">{item.currentProgress}%</span>
                        </div>
                        <div className="w-full h-1.5 bg-[#EDF2F7] rounded-full overflow-hidden">
                          <div
                            className="h-full bg-[#1B3F18] rounded-full"
                            style={{ width: `${Math.min(100, item.currentProgress)}%` }}
                          ></div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 text-[11px] text-[#718096] mt-1.5">
                        <span className="flex items-center gap-1">
                          <Users size={11} />
                          {item.donorCount} apoiadores
                        </span>
                        <span className="truncate">Até {item.deadline}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-2 pt-3 mt-3 border-t border-[#F0F0F0]">
                    <button
                      type="button"
                      onClick={() => toggleCampaignPublication(item.id)}
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        if (confirm(`Deseja realmente remover a campanha "${item.title}"?`)) {
      ...
      {showNoticeModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-[#E2E8F0] animate-scaleUp text-center">
            <div className="w-14 h-14 rounded-2xl bg-rose-50 text-rose-800 flex items-center justify-center mx-auto mb-3">
              <Info size={28} />
            </div>
